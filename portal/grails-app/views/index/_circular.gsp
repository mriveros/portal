...
<body>
	...
	<div class="panel panel-default">
		<div class="panel-body" id="panelBody_circular">
			<div class="row">
		...
		<div class="panel-footer" style="height: 30%;">
			<div style="width: 100%" id="panelFooter_circular"></div>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		function getDatos(index){
	        ...
	        $.ajax({
				...
				success : function(rs) {
					var list = rs.split("|");
					console.log(list);
					$("#tb_body_circular").html(list[0]);
					initPaginador(list[1], list[2], index);
					if (list[2] == "0") {
						$('#panelFooter_circular').html("");
					}
				}
			})
		...
		function initPaginador(cantPag, cantReg, current) {
			...
			$('#panelFooter_circular').smartpaginator({
